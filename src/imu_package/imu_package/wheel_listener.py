...
class MyNode(Node):
	...
	def __init__(self):
		...
		self.frequency = 0.01 	#Period between callbacks
		...
		self.subscribtion_left = self.create_subscription(Int16, '/motors/measure/encoder_1', self.callback_right, 10)
		self.subscribtion_right = self.create_subscription(Int16, '/motors/measure/encoder_2', self.callback_left, 10)
		self.speed_right = 0
		self.speed_left  = 0
		#self.conversion_encodeur_exit = 156*2*np.pi/(670*60)
		self.conversion_encoder_deplacement = lambda enc: ((enc*10/51)/(2*np.pi))*0.06*np.pi*1.5
	...
	def timer_callbacks(self):
		...
		odom.twist.twist.angular.z = (self.speed_right - self.speed_left)/0.019
